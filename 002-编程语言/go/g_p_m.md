...
top:
    ...
    // 安全检查，如果 MP 自旋状态，则 runnext 与本地 runq 必须为空。
    // 此检查在 checkTimers (在 findRunnable 中) 之前检查，因为 checkTimers 可能会调用 goready 把一个 goroutine 放入本地 runq。
    ...
    // tryWakeP 为 true 表示获取的 G 不一般 (GC worker, trace reader)
    // 尝试唤醒 P，以保证有足够的线程来调度 GCworker 和 tracereader。
    if tryWakeP {
        wakep()
    }
    // 如果 G 绑定了 M，唤醒 M 来执行 G。
    if gp.lockedm != 0 {
...
```go
// 找一个可执行的 G 来执行。
// tryWakeP 表示返回的 G 不一般 (GC worker, trace reader) 所以调用者应该尝试唤醒一个 P。
